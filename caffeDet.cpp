...
bool Detector::DetectImg(cv::Mat& cv_img, vector<cv::Rect>& found) {
    float CONF_THRESH = hp->readDoubleParameter("Conf.det_conf_thresh");
    ...
    for (int h = 0; h < cv_img.rows; ++h )
    {
        ...
        {
            cv_new.at<cv::Vec3f>(cv::Point(w, h))[0] = 
              float(cv_img.at<cv::Vec3b>(cv::Point(w, h))[0])-float(102.9801);
            cv_new.at<cv::Vec3f>(cv::Point(w, h))[1] = 
              float(cv_img.at<cv::Vec3b>(cv::Point(w, h))[1])-float(115.9465);
            cv_new.at<cv::Vec3f>(cv::Point(w, h))[2] = 
              float(cv_img.at<cv::Vec3b>(cv::Point(w, h))[2])-float(122.7717);
    ...
    for (int h = 0; h < height; ++h )
    {
        ...
        {
            data_buf[(0*height+h)*width+w] = 
              float(cv_resized.at<cv::Vec3f>(cv::Point(w, h))[0]);
            data_buf[(1*height+h)*width+w] = 
              float(cv_resized.at<cv::Vec3f>(cv::Point(w, h))[1]);
            data_buf[(2*height+h)*width+w] = 
              float(cv_resized.at<cv::Vec3f>(cv::Point(w, h))[2]);
        }
    }
    ...
    {
        if (i != 15) {  // get results in person category
            continue;
        }
        for (int j = 0; j< num; j++)
        {
        ...
        _nms(keep, &num_out, sorted_pred_cls, num, 5, NMS_THRESH, 
             hp->readIntParameter("Conf.GPUID"));
        //for visualize only
    ...
    // return sorted_pred_cls[keep[1]*5+4];  // score
    return true;
}
...
void Detector::boxes_sort(const int num, const float* pred, float* sorted_pred) {
    ...
    std::sort(my.begin(), my.end(), compInfo);
    for (int i=0; i<num; i++)
    {
        for (int j=0; j<5; j++)
...
void Detector::getBBox(vector<cv::Rect>& found, int* keep, int num_out, 
                       float* sorted_pred_cls, float CONF_THRESH) {
    found.clear();
    int i = 0;
    for(; sorted_pred_cls[keep[i]*5+4]>CONF_THRESH && i < num_out; i++)
    {
        cv::Rect rect(cv::Point(sorted_pred_cls[keep[i]*5+0], sorted_pred_cls[keep[i]*5+1]),
                      cv::Point(sorted_pred_cls[keep[i]*5+2], sorted_pred_cls[keep[i]*5+3])); 
...
void Detector::bbox_transform_inv(int num, const float* box_deltas, 
                                  const float* pred_cls, float* boxes, 
                                  float* pred, int img_height, int img_width) {
  float width, height, ctr_x, ctr_y, dx, dy, dw, dh;
